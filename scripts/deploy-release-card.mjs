...
const source = await readFile(resolve(process.cwd(), "contracts/ReleaseCard.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "ReleaseCard.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["ReleaseCard.sol"]?.ReleaseCard;
...
console.log(`Deploying ReleaseCard from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_RELEASE_CARD_CONTRACT_ADDRESS to this contract address in Vercel.");
